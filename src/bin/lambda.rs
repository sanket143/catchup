...
use juniper::{EmptySubscription, http::GraphQLResponse};
use juniper::{execute, http::GraphQLRequest};
use lambda_http::{Body, Error, IntoResponse, Request, Response, http, run, service_fn};
use lazy_static::lazy_static;

#[tokio::main]
async fn main() -> Result<(), Error> {
    dotenvy::dotenv().ok();
    db::create_db_connection().await.unwrap();
